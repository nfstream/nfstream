...
nf_packet = namedtuple('NFPacket', [
    'time', 'delta_time', 'direction', 'raw_size', 'ip_size', 'transport_size',
    'payload_size', 'src_ip', 'src_mac', 'src_oui', 'dst_ip', 'dst_mac',
    'dst_oui', 'src_port', 'dst_port', 'protocol', 'vlan_id', 'ip_version',
    'ip_packet', 'syn', 'cwr', 'ece', 'urg', 'ack', 'psh', 'rst', 'fin',
    'tunnel_id'
])
...
class NFlow(object):
    ...
    def __init__(self, packet, ffi, lib, udps, sync, accounting_mode,
        ...
        self.src_ip = ffi.string(self._C.src_ip).decode('utf-8',
                                                        errors='ignore')
        self.src_mac = ffi.string(self._C.src_mac).decode('utf-8',
                                                          errors='ignore')
        self.src_oui = ffi.string(self._C.src_oui).decode('utf-8',
                                                          errors='ignore')
        self.src_port = self._C.src_port
        self.dst_ip = ffi.string(self._C.dst_ip).decode('utf-8',
                                                        errors='ignore')
        self.dst_mac = ffi.string(self._C.dst_mac).decode('utf-8',
                                                          errors='ignore')
        self.dst_oui = ffi.string(self._C.dst_oui).decode('utf-8',
                                                          errors='ignore')
        ...
        if n_dissections:  # Same for dissection when > 0
            self.application_name = ffi.string(
                self._C.application_name).decode('utf-8', errors='ignore')
            self.application_category_name = ffi.string(
                self._C.category_name).decode('utf-8', errors='ignore')
            self.application_is_guessed = self._C.guessed
            self.requested_server_name = ffi.string(
                self._C.requested_server_name).decode('utf-8', errors='ignore')
    ...
    def update(self, packet, idle_timeout, active_timeout, ffi, lib, udps,
               sync, accounting_mode, n_dissections, statistics, splt,
               dissector):
        """ NFlow update method """
        # First, we update internal C structure.
        ret = lib.meter_update_flow(self._C, packet, idle_timeout,
                                    active_timeout, accounting_mode,
                                    statistics, splt, n_dissections, dissector)
        if ret > 0:  # If update done it will be zero, idle and active are matched to 1 and 2.
            self.expiration_id = ret - 1
            return self.expire(udps, sync, n_dissections, statistics, splt,
                               ffi, lib, dissector)  # expire it.
    ...
    def expire(self, udps, sync, n_dissections, statistics, splt, ffi, lib,
               dissector):
        ...
        lib.meter_free_flow(self._C, n_dissections, splt,
                            1)  # then free C struct
        ...
        if statistics:  # Statistical analysis activated
            ...
            if bidirectional_packets > 1:
                self.bidirectional_stddev_ps = sqrt(
                    self._C.bidirectional_stddev_ps /
            ...
            if src2dst_packets > 1:
                self.src2dst_stddev_ps = sqrt(self._C.src2dst_stddev_ps /
                                              (src2dst_packets - 1))
            self.src2dst_max_ps = self._C.src2dst_max_ps
            self.dst2src_min_ps = self._C.dst2src_min_ps
            self.dst2src_mean_ps = self._C.dst2src_mean_ps
            ...
            if dst2src_packets > 1:
                self.dst2src_stddev_ps = sqrt(self._C.dst2src_stddev_ps /
                                              (dst2src_packets - 1))
            self.dst2src_max_ps = self._C.dst2src_max_ps
            self.bidirectional_min_piat_ms = self._C.bidirectional_min_piat_ms
            self.bidirectional_mean_piat_ms = self._C.bidirectional_mean_piat_ms
            if bidirectional_packets > 2:
                self.bidirectional_stddev_piat_ms = sqrt(
                    self._C.bidirectional_stddev_piat_ms /
        ...
        if n_dissections:  # If dissection set (>0)
            ...
            if self._C.detection_completed < 2:
                ...
                self.application_category_name = ffi.string(
                    self._C.category_name).decode('utf-8', errors='ignore')
                self.requested_server_name = ffi.string(
                    self._C.requested_server_name).decode('utf-8',
                                                          errors='ignore')
                self.client_fingerprint = ffi.string(self._C.c_hash).decode(
                    'utf-8', errors='ignore')
                self.server_fingerprint = ffi.string(self._C.s_hash).decode(
        ...
        if splt:
            if sync_mode:  # Same for splt, once we reach splt limit, there is no need to sync it anymore.
                if self._C.bidirectional_packets <= splt:
                    self.splt_direction = ffi.unpack(self._C.splt_direction,
                                                     splt)
                    self.splt_ps = ffi.unpack(self._C.splt_ps, splt)
                    self.splt_piat_ms = ffi.unpack(self._C.splt_piat_ms, splt)
                else:
    ...
    def __str__(self):
        ...
        for attr_name in self.__slots__:
            try:
                if not started:
                    printable += attr_name + "=" + str(getattr(
                        self, attr_name))
                    started = True
                else:
                    if attr_name == 'udps':
